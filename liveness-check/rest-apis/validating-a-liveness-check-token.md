# Validating a Liveness Check Token

## **POST `/v1/token/validate`**

The `/v1/token/validate` endpoint accepts a `LivenessCheckToken` generated from `/v1/card/verify` endpoint and returns whether the Card Verify attempt corresponding to that token is valid or not. On invalid requests, the response will contain a list of failure\_reasons around why Bouncer believes this Card Verify scan to be suspicious.
...
`token: string` LivenessCheckToken from the `/v1/card/verify` endpoint
...
* `pan_not_found` Present if the Liveness Check scan did not pick up a valid PAN from either the front or the back of the card. 
