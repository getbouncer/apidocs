...
You may need to support cards which do not come bundled with the Liveness SDK by default, this can be done with the addition of a Custom Issuer.
...
Some cards have very specific ways of arranging the numbers when they are displayed. The Liveness SDK has an option that allows for the formatting of custom pans. Just use the following method:
