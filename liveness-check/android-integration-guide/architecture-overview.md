...
The Liveness framework operates like a pipeline. Images from a data source \(e.g. a Camera\) are processed by a pool of
...
First and foremost, Liveness is used to process images from a device camera. The
...
card number. The Liveness SDK uses a `ResultAggregator` to run a voting algorithm on the results from each camera frame.
...
that multiple analyzers are created, and then used as needed. The Liveness SDK creates a pool of analyzers at the
